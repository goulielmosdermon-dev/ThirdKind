...
/**
 * A piece of music, as one button and nothing else.
 *
 ...
 *
 * Once the reader is past it the same control follows them down the page,
 * held to the corner, so the music can be stopped from wherever they have got
 * to rather than by going back to find it.
 */
export function SoundBar({
  ...
  src: string;
  /** Not shown: it names the control for anyone who cannot see it. */
  title: string;
  className: string;
}) {
  ...
  */
  ...
  const away = slide === null ? undefined : slide !== 'in';
  ...
  return (
    <div ref={barRef} className={className}>
      ...
      <button
        ...
        className="cursor-pointer border-0 bg-transparent p-0 text-ink transition-opacity duration-300 hover:opacity-60"
      >
        {playing ? (
          <PauseGlyph className="h-[clamp(3rem,7vw,5.5rem)] w-auto" />
        ) : (
          <PlayGlyph className="h-[clamp(3rem,7vw,5.5rem)] w-auto" />
        )}
      </button>

      {/* The same control, kept to the corner once the track is behind the
          reader, and only worth showing while there is something to stop.
          ...
          the pointer by the time this is wanted. Both flags start false, so
          the server and the first client render agree on nothing being here. */}
      {typeof document !== 'undefined' && (away ?? past) && started
        ? createPortal(
            <button
              type="button"
              onClick={() => void toggle()}
            ...
            >
              {playing ? (
                <PauseGlyph className="h-5 w-auto" />
              ) : (
                <PlayGlyph className="h-5 w-auto" />
              )}
            </button>,
            document.body,
          )
